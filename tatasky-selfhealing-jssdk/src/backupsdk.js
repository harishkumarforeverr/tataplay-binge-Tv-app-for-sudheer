...
//------------------------------------------------------------------------------------
    ...
    _getPingEvent(playbackDuration) {
        ...
        // console.log('forward ==abcdef--SBL' + JSON.stringify(_player.buffer.getLevel('forwardduration', 'video')));
        // console.log('backward--abcdef--RBL' + JSON.stringify(_player.buffer.getLevel('backwardduration', 'video')));
       // console.log('--abcdef--bitrate--' + JSON.stringify(_player.getDownloadedVideoData().bitrate));
    ...
    _onSeeked(e) {
       // console.log('Rebuffering-Buffer-Length (seconds): --abcde--' + player.buffer.getLevel('forwardduration', 'video').targetLevel); 
        ...
        if (response != null && response.registration_response != null) {
            ...
            if (response.registration_response.cfg != null && response.registration_response.cfg != undefined && response.registration_response.cfg.mc != null && response.registration_response.cfg.mc != undefined) {
                if(response.registration_response.cfg.mc.estimatedDownloadRate ==-1 && response.registration_response.cfg.mc.startupBuffDuration == -1 && response.registration_response.cfg.mc.rebufferingDuration == -1){
                    ...
                    this._persistKIntVP(KLocalStorageKeyDownloadRate, 40000)    
    ...
    onVideoAdaptationSetter(e) {
        console.log("onVideoAdaptation=-",e);
        if (isInStartup) {
            if (!availableQualities) {
                availableQualities = _player.getAvailableVideoQualities();
            ...
            for (var i = availableQualities.length - 1; i >= 0; i--) {
                if (availableQualities[i].id === e?e.suggested:0) {
                    // Use the suggested quality if it's higher than the preferred
                    return e?e.suggested:_initialDownloadRate / 1000;
                }
    ...
    getMitigationConfiguration(playerConfig) {
     ...
     try {
        ...
        if (this._getMitigationConfigurationAvailable() == true) {
           console.log("abcde--1-miti-", playerConfig);
           console.log("abcde--2-miti-",_reBufferThreshold);
            ...
            // playerConfig.adaptation.tweaks.restart_threshold = _startupBufferThreshold  //this._getIntV(KLocalStorageKeyStartupThreshold)
           ...
           // console.log("onVideoAdaptation--",this.onVideoAdaptationSetter());
            // playerConfig.adaptation.desktop.onVideoAdaptation = this.onVideoAdaptationSetter();
            // playerConfig.adaptation.mobile.onVideoAdaptation = this.onVideoAdaptationSetter();
