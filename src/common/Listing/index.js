...
import {
    ...
    getProviderLogo,
    isMobile,
    cloudinaryCarousalUrl, isPartnerSubscribed,
...
class Listing extends Component {
    ...
    handleSwipeEvent = () => {
        ...
        for (let item = 0; item < items.length; item++) {
            ...
            if (element.length > 0) {
                detectSwipe(className, (el, dir, data) => {
                    if (dir === DIRECTIONS.LEFT && this.state?.swipeSliderRailId !== data?.id) {
    ...
    handleOnResize = () => {
        ...
        this.state.visitedSliderID &&
            this.state.visitedSliderID.map((item) => {
                let showItemCount = slidesToShow(item.layoutType, item.sectionSource, !!isMobile.any());
                let leftItemsCount =
    ...
    getProviderLogoImage = (provider) => {
        if (provider) {
            ...
            providerImageUrl = providerImage && providerImage[provider].logoRectangular;
            return providerImageUrl;
        }
    };

    getListingClass = (railItem) => {
        if ([SECTION_SOURCE.LANGUAGE, SECTION_SOURCE.GENRE].includes(railItem.sectionSource)) {
        ...
        else if (railItem?.sectionSource === SECTION_SOURCE.BINGE_DARSHAN_CHANNEL) {
    ...
    getClassBasedOnSource = (railItem) => {
        return `listing-vertical rail-${railItem.id} ${this.props.addClass}
                ${railItem.sectionSource === SECTION_SOURCE.BINGE_TOP_10_RAIL ? 'binge-top-rail' : ''}
                ${railItem.sectionSource === SECTION_SOURCE.BACKGROUND_BANNER_RAIL ? 'background-banner-rail' : ''}`;
    };
    ...
    filterRefUseCase = (id) => {
        ...
        const obj = !isEmpty(heiarchchyData) && heiarchchyData.filter(data => id === data.railId);
        return get(obj, '[0].refId', '');
    }

    renderRail = (railItem, railPosition) => {
        console.log('railItem', railItem)
        const { pageType, isPartnerPage, taRecommendedRail, pidetailpage, customContents, isSeasons } =
            this.props;
        const toolTipToShow = [LAYOUT_TYPE.TOP_PORTRAIT, LAYOUT_TYPE.LANDSCAPE, LAYOUT_TYPE.PORTRAIT].includes(railItem.layoutType);
        const notShowToolTip = ![SECTION_SOURCE.BINGE_CHANNEL, SECTION_SOURCE.GENRE, SECTION_SOURCE.LANGUAGE, SECTION_SOURCE.BACKGROUND_BANNER_RAIL, SECTION_SOURCE.SEASONS, SECTION_SOURCE.CATEGORY, SECTION_SOURCE.BINGE_DARSHAN_CHANNEL].includes(railItem.sectionSource);
        const refUseCase = this.filterRefUseCase(railItem?.id);
        const isBackgroundRailProviderEmpty = !(!!railItem?.provider);
        return railItem.contentList &&
                ...
                return (
                    <ListingItem
                        ...
                        isContinueWatching={
                            ...
                            SECTION_SOURCE.CONTINUE_WATCHING
                        }
                        scrollPosition={this.props.scrollPosition}
                        handleOnMouseDown={this.handleOnMouseDown}
                        onClickHandler={this.onRailItemClick}
                        item={railItem.sectionSource === SECTION_SOURCE.SEASONS ? { ...item, image: item.boxCoverImage } : item}
                        key={index}
                        ...
                        taRecommendedRail={taRecommendedRail}
                        subscribed={isUserloggedIn() && this.isPartnerSubscribed(
                            railItem.layoutType,
                            item.partnerId,
                            railItem.sectionSource,
                        )}
                        ...
                        isAbsolute={false}
                        classNameSetHover={isSeasons ? 'season-hover' : classNameToApply('set-portrait-hover', LAYOUT_TYPE.PORTRAIT, railItem.layoutType)}
    ...
    showLanguageNudge = (sectionSource) => {
        ...
        if (sectionSource === SECTION_SOURCE.LANGUAGE_NUDGE && isEmpty(userLanguage)) {
            return <Languages />;
        }
        return null;
    }
    setContainerClassName = () => {
        ...
        return `
       ${(isHomePage && !heroBannerItems?.length > 0 && !isMobile.any()) ? "margin-top-30" : ""}`;
    ...
    trackMixpanelEvent = (railItem, updatedRailPosition) => {
        ...
        let pageName = location?.state?.setPathIs?.state?.partnerName ? location?.state?.setPathIs?.state?.partnerName : location?.state?.source || getAnalyticsSource(this.props.location.pathname, MIXPANEL)
        mixPanelConfig.trackEvent(MIXPANEL.EVENT.VIEW_RAIL_CONTENT, {
            [MIXPANEL.PARAMETER.RAIL_TITLE]: railItem.sectionSource === SECTION_SOURCE.SEASONS ? MIXPANEL.VALUE.SEASONS : railItem?.title,
    ...
    getTitleClassName = (railItem) => {
        ...
        } else if (railItem.sectionSource === SECTION_SOURCE.BINGE_CHANNEL || railItem.sectionSource === SECTION_SOURCE.BINGE_DARSHAN_CHANNEL) {
    ...
    render() {
        const { items } = this.props,
            railItems = items && items.filter((item) => item.sectionType !== SECTION_TYPE.HERO_BANNER);
        ...
        let railPosition = 0;
        return (
            <ul className={`listing-container rail-container ${this.setContainerClassName()}
            ${categoryDropdownHeader(this.props.location) && `category-page-listing`}
            ${railItems?.length > 0 && railItems[0].sectionSource === SECTION_SOURCE.LANGUAGE_NUDGE && this.showLanguageNudge(railItems[0].sectionSource) && 'language-nudge'}`}>
                {railItems?.length > 0 && railItems.map((railItem, railIndex) => {
                    if (railItem.sectionSource !== SECTION_SOURCE.LANGUAGE_NUDGE) {
                        railPosition++;
                    }
                    const updatedRailPosition = railPosition;
                    return <React.Fragment key={railIndex}>
                        {railItem.sectionSource === SECTION_SOURCE.LANGUAGE_NUDGE
                            ...
                            (
                                ...
                                >
                                    <>  Hiiiiiiiiiiiiiii
                                        <div
                                            className={`${railItem.sectionSource === SECTION_SOURCE.BACKGROUND_BANNER_RAIL && `sports-rail-wrapper`}`}>
                                            {railItem.sectionSource === SECTION_SOURCE.BACKGROUND_BANNER_RAIL &&
                                                <div className="sports-rail-background">
                                                    <img alt=""
                                                        src={`${cloudinaryCarousalUrl('', '', sportsBannerWidth * 2, sportsBannerHeight * 2)}${railItem.backgroundImage}`}
                                                    />
                                                </div>
                                            }
                                            <div
                                                className={this.getTitleClassName(railItem)}>
                                                {railItem?.contentList?.length > 0 && <div className={`divider-title-section`}>
                                                    {(railItem?.provider === "" || railItem?.provider == null) && isMobile.any() && railItem?.sectionSource === SECTION_SOURCE.BACKGROUND_BANNER_RAIL ? (
                                                        null
                                                    ) :
                                                        <>
                                                            {railItem?.sectionSource === SECTION_SOURCE.BACKGROUND_BANNER_RAIL && isMobile.any() ?
                                                                <img
                                                                    className="sports-rail-header-image"
                                                                    src={`${this.getProviderLogoImage(railItem?.provider)}`}
                                                                    alt='sport-tilte' />
                                                                : <span className="rail-title">
                                                                    <h3>{railItem.title}</h3>
                                                                    {(railItem?.sectionSource === SECTION_SOURCE.BINGE_CHANNEL || railItem?.sectionSource === SECTION_SOURCE.BINGE_DARSHAN_CHANNEL) && <div className="red-dot"><img src={LiveDot} className="livedot-image" /></div>}
                                                                </span>
                                                            }
                                                        </>
                                                    }
                                                    {this.isSeeAllLinkVisible(railItem) ? (
                                                        <span
                                                            className={`see-all-link    ${railItem.sectionSource ===
                                                                SECTION_SOURCE.PROVIDER && `see-all-margin`
                                                                }`}
                                                            onClick={() =>
                                                                this.seeAll(
                                                                    this.getSeeAllVariables(railItem, updatedRailPosition),
                                                                )
                                                            }
                                                        >
                                                        </span>
                                                ...
                                                <ul
                                                    className={`listing-horizontal ${this.getListingClass(
                                                        railItem,
                                                    )} ${railItem.sectionSource === SECTION_SOURCE.SEASONS && 'seasons-block'}`}
                                                    key={railItem.id}
                                                    ref={(input) => {
                                                ...
                                                >
                                                    {appWidth > MOBILE_BREAKPOINT ?
                                                            ...
                                                            <Slider
                                                                ...
                                                                {...{
                                                                    ...
                                                                    slidesToScroll: this.state[
                                                                        ...
                                                                        ? this.state[`slidesToScroll_${railItem.id}`]
                                                                        : getDefaultValue(
                                                                            railItem.layoutType,
                                                                            railItem.contentList.length,
                                                                            railItem.sectionSource,
                                                                ...
                                                                afterChange={(item, e) =>
                                                                    this.handleAfterChange(item, e, railItem, updatedRailPosition)
                                                                }
                                                                ...
                                                                <div>
                                                                    <span class="listing-block">
                                                                        ...
                                                                        >
                                                                            ...
                                                                            >
                                                                                <div
                                                                                    className="shuffleCardContainer innerCard_two"

                                                                                >
                                                                                    <div className="shuffleCardContainer innerCard_three"


                                                                                    >
                                                                                        <div
                                                                                            className="shuffleCardContainer innerCard_four"
                                                                                        >
                                                                                            <div>
                                                                                                <div
                                                                                                className="shuffleCard_content"
                                                                                                    // style={{
                                                                                                    //     display: "flex",
                                                        ...
                                                        :
                                                        <div className={`horizontal-rail railItem-${railItem.id}`}
                                                            onTouchStart={() => this.getRailPositionValue(updatedRailPosition)}>
                                                            {this.renderRail(railItem, railPosition)}
                                                        </div>
                                                    }
                                                </ul>
