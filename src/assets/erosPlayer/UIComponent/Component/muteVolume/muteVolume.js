...
function logituitMuteVolumeComponent() {
    ...
    this.muteVolume = videojs.extend(videojs.getComponent('Button'), {
      ...
      tooltipDataFun: function(){
        ...
        tooltipData.setAttribute("class", "logituit_muteVolumeTooltip");
        tooltipData.innerHTML = "Mute";
        me.el().appendChild(tooltipData);
      },
       createEl: function() {
       this.elementRef = videojs.dom.createEl('button', {
         className:'logituit_MuteVolumePanel'
       });
    ...
    volumeBarMouseOverShow:function(){
      this.el().addEventListener("mouseenter",function(event){
      var mousehover=document.getElementsByClassName('logituit_volumeSlider')[0];
     // this.el().addEventListener("mouseenter",function(event){
        mousehover.style.visibility="visible";
      })
    },

    volumeBarMouseOverHide:function(){
      this.el().addEventListener("mouseout",function(event){
      var mousehover=document.getElementsByClassName('logituit_volumeSlider')[0];
      //this.el().addEventListener("mouseout",function(event){
        mousehover.style.visibility="hidden";
      })
    },
    ...
    videojs.registerComponent('LogixMuteVolume', this.muteVolume);
    }